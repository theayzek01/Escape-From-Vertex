...
namespace our
{
    ...
    public:
        ...
        HMODULE hIrrKlang = LoadLibraryA("irrKlang.dll");
        if (!hIrrKlang) {
            hIrrKlang = LoadLibraryA("vendor/Irrklang/dlls/irrKlang.dll");
        }
        if (hIrrKlang) {
            typedef irrklang::ISoundEngine* (*PFN_createIrrKlangDevice)(
                irrklang::E_SOUND_OUTPUT_DRIVER driver,
                int options,
                const char* deviceID,
                const char* sdk_version_do_not_use
            );
            PFN_createIrrKlangDevice pfn_createIrrKlangDevice = (PFN_createIrrKlangDevice)GetProcAddress(
                hIrrKlang, 
                "?createIrrKlangDevice@irrklang@@YAPEAVISoundEngine@1@W4E_SOUND_OUTPUT_DRIVER@1@HPEBD1@Z"
            );
            if (pfn_createIrrKlangDevice) {
                SoundEngine = pfn_createIrrKlangDevice(
